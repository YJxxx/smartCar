...
extern void     tpm_input_init(TPMn_e, TPM_CHn_e, TPM_Input_cfg cfg,TPM_PS_e ps);   //输入捕捉初始化
extern uint16   tpm_input_get   (TPMn_e, TPM_CHn_e);                                //输入捕捉模式下，读取捕捉事件发生时的计数器值(根据两次捕捉之间的差值可判断周期频率)
extern void     tpm_input_clean (TPMn_e );                                          //清空计数器的值
